...
const PROTOCOL_CONTENT = {
  'data-entry': {
    ...
    body: (
      <>
        <p className="text-slate-700 mb-2">Welcome,</p>
        <p className="text-slate-700 mb-2">You are currently in practice mode</p>
        <p className="text-slate-700 mb-6">
          Want to switch? <button type="button" className="text-[#2563eb] underline font-medium">Click here to sign in.</button>
        </p>
  ...
  clouds: {
    ...
    body: (
      <>
        ...
        <p className="text-slate-700 mb-4">
          Users can visualize the observations you submit to GLOBE, as well as those submitted by other app users from around the world, by clicking <button type="button" className="text-[#2563eb] font-semibold">HERE</button>.
        </p>
        <p className="text-slate-700 mb-4">
        ...
        <p className="text-slate-700 mb-6">
          In addition, observing at solar noon allows you to more easily compare your measurements with other GLOBE users. You can also set a notification for solar noon at your location. Notifications are enabled by selecting the settings icon <Settings className="inline w-4 h-4 text-slate-600" /> in the top right corner of the Home screen.
        </p>
        <button type="button" className="flex items-center justify-between w-full py-3 text-[#2563eb] font-semibold border-t border-sky-200 mt-2">
          Next
  ...
  trees: {
    ...
    body: (
      <>
        <h2 className="text-xl font-bold text-slate-800 mb-4">Welcome to the GLOBE Observer Trees App</h2>
        <p className="text-slate-700 mb-4">
  ...
  if (!content) {
    return (
      <div className="space-y-4">
        <button type="button" onClick={() => navigate('/')} className="flex items-center gap-2 text-[#2563eb] font-medium">
          <ArrowLeft className="w-5 h-5" /> Back to Home
        </button>
        <p className="text-slate-600">Protocol not found.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 pb-8">
      <button
        type="button"
        onClick={() => navigate('/')}
        className="flex items-center gap-2 text-[#2563eb] font-medium hover:underline"
      >
